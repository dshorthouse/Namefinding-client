...
class TaxonFinderClient < FinderClient
  def initialize(config_file="taxon_config.yml")
    super
  ...
  def taxon_find(word)
    ...
    if output = @socket.gets
      ...
      unless response.return_string.blank?
        ...
        add_name Name.new(verbatim_string, :start_position => @document.rindex(verbatim_string), :scientific_name => scientific_string)
      end
